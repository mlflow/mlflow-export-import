...
Options:
  --dataset-name TEXT  Name of the evaluation dataset to export (Databricks/Unity Catalog).
  --dataset-id TEXT    ID of the evaluation dataset to export (open-source MLflow).
  --output-dir TEXT    Output directory.  [required]

Note: Either --dataset-name or --dataset-id must be provided (mutually exclusive).
- In Databricks: Use --dataset-name with Unity Catalog table name (e.g., catalog.schema.dataset)
- In open-source MLflow: Use --dataset-id with the dataset UUID
```

#### Examples

##### Export by dataset name (Databricks)
```
export-evaluation-dataset \
  --dataset-name catalog.schema.wine-quality-eval \
  --output-dir out
```

##### Export by dataset ID (open-source MLflow)
```
export-evaluation-dataset \
  --dataset-id d-7f2e3a9b8c1d4e5f6a7b8c9d0e1f2a3b \
  --output-dir out
```
