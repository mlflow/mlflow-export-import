...
def _export_experiment(mlflow_client, exp_id_or_name, output_dir, exporter, export_results, run_ids):
...
def export_experiments(
    ...
    export_results = []
    futures = []
    exporter = ExperimentExporter(mlflow_client, notebook_formats=utils.string_to_list(notebook_formats))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for exp_id_or_name in experiments:
            run_ids = experiments_dct.get(exp_id_or_name, None)
            future = executor.submit(_export_experiment, mlflow_client, exp_id_or_name, output_dir, exporter, export_results, run_ids)
