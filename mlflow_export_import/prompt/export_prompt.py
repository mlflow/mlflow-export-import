...
def _get_prompt_safe(prompt_name, prompt_version):
    """Get prompt with version-aware API calls - ensures compatibility across MLflow deployments."""
    ...
    try:
        ...
        if hasattr(client, 'load_prompt'):
            return client.load_prompt(prompt_name, prompt_version)
    except (ImportError, AttributeError, Exception):
        pass
    
    # Try older get_prompt API (MLflow 2.21+)
    try:
        if hasattr(mlflow, 'get_prompt'):
            return mlflow.get_prompt(prompt_name, prompt_version)
    except (ImportError, AttributeError, Exception):
        pass
