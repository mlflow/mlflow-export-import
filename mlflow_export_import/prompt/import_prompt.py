...
def _check_import_compatibility(prompt_data):
    ...
    from mlflow_export_import.common.version_utils import get_mlflow_version, get_version_info
...
def _create_prompt_safe(name, template, tags=None, commit_message=None):
    """Create prompt with version-aware API calls - ensures compatibility across MLflow deployments."""
    ...
    try:
        ...
        _logger.debug(f"mlflow.genai.register_prompt failed for '{name}': {e}")
    ...
    try:
        ...
        _logger.debug(f"mlflow.register_prompt failed for '{name}': {e}")
    ...
    try:
        ...
        _logger.debug(f"client.register_prompt failed for '{name}': {e}")
    
    # Try older create_prompt API (MLflow 2.21+)
    try:
        if hasattr(mlflow, 'create_prompt'):
            return mlflow.create_prompt(
                name=name,
                template=template,
                tags=tags or {}
            )
    except (ImportError, AttributeError) as e:
        _logger.debug(f"mlflow.create_prompt not available: {e}")
    except Exception as e:
        _logger.debug(f"mlflow.create_prompt failed for '{name}': {e}")
...
def main(input_dir, prompt_name):
