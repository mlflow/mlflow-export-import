...
class DatabricksTester():
    ...
    def run_export_experiment_job(self):
        ...
        exp_name = os.path.join(self.ws_base_dir, _experiment_name)
        notebook_task = { 
            "notebook_path": nb_path,
            "base_parameters": {
              " Experiment ID or Name": exp_name,
              "Destination base folder": self.dst_exp_base_dir,
              "Export metadata tags": _export_src_tags,
              "Registered model": self.model_name
            }
        }
        run_name = self._mk_run_name(nb_name)
        if self.verbose:
            self._dump_json("run_export_experiment_job - notebook_task", notebook_task)
