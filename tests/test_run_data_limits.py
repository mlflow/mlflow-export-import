...
from utils_test import create_experiment, create_dst_experiment_name
...
def test_params(mlflow_context):
    run1, run2 = init_test_runs(mlflow_context, 
...
def init_test_runs(mlflow_context, exporter, importer, num_params=None, num_metrics=None, num_tags=None):
    exp, run = create_run(mlflow_context.client_src, num_params, num_metrics, num_tags)
    exporter.export_run(run.info.run_id, mlflow_context.output_dir)

    experiment_name = create_dst_experiment_name(exp.name)
    res = importer.import_run(experiment_name, mlflow_context.output_dir)
